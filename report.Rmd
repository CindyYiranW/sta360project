...
We generate 12 posterior distributions each on the parameters $\beta$ and $\alpha$. 

## Posterior predictive and checks

MCMC samples of parameters $\beta$ and $\alpha$ from 12 posterior distributions using metropolis algorithm. Since our goal is to predict arrest rate for this year, we could simulate the predictive distribution $[Z|data]$ using the posterior distribution $[\beta|data]$. Based on the joint distribution of Z, $\beta$ and $\alpha$: 
...
\]
 ...
 Ignoring $\beta$, we generate posterior predive samples samples $Z_1$, ..., $Z_S$. 
