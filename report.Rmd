...
![A box plot of all 75 precincts showing the variation in stops](images/ran_effect.png)
...
Below are the posterior predictive checks for violent crimes in precincts with less than 10% African American population. Our model captures the majority of the actual stops well, but doesn't have a very accurate grasp on the outliers. The variances of our predicted stops also varies more than we expected, but the actual mean is centered around the middle point of our predicted cases, so we think our model is a good fit overall.
...
1) regardless of precinct black population percentage, black people are more often stopped for all crime types except for property crimes

2) for precincts with 10-40% black population, whites are 3x less likely stopped for violent crimes 

3) for property crime in a precinct with more than 40% black population, whites are always more likely to be targeted than other races within a 95% credible interval 

4) for precincts with over 40% black population, black people are 1.4-1.5x more suspectible from stopping from drug crimes than hispanics and whites. 
