...
```{r regression}
stops<-data$stops ; ethi<-as.factor(data$eth) ; as.factor(precinct<-data$precinct);arrest=data$past.arrests
...
for (i in 1:n.precinct) {
  precincts[precinct.original==precinct.number[i]] <- i
...
for (j in 1:3){
  for (k in 1:4){
    ...
    M1[[index]] <- glmer(stops ~ dcjs + (1|ethi) + (1|precincts),
     family=poisson(link="log"), subset=ok, data=stop_df)
    M2[[index]] <- glmer.nb(stops~1+ethi+(1|precincts), verbose=TRUE, subset=ok,data=stop_df)
  }
}
M1[1]
M2[1]
